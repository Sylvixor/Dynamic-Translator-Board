//the following code is based on the raspberry pi pico example code 
#include <sensor.h>
...
DynamicJsonDocument sensorConstructJson(){
    ...
    doc["d0"] = (bmp280_convert_temp(raw_temperature, &params) / 100.f);
    doc["d1"] = (bmp280_convert_pressure(raw_pressure, raw_temperature, &params) / 1000.f);
    return doc;
}
...
// CyrbSensor class implementations
...
WriteStruct CyrbSensor::sensorPrepData(){
    ...
    DynamicJsonDocument doc = sensorConstructJson();
    serializeJson(doc, ws.buffer, sizeof(ws.buffer));
